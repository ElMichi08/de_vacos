...
|-------|-------------|-----------|-----------------------------|------------|
| **Numeración 1-n (diario)** | Los números de orden se generan secuencialmente desde 1 hasta n, sin límite superior ni ciclo. El contador se reinicia implícitamente al cambiar de día (nuevo día → primer pedido recibe #1). Algoritmo: `MAX(numeroOrden HOY, incluyendo cancelados) + 1`. | `lib/core/database/db_helper.dart:_calcularSiguienteNumeroOrden` | Números repetidos en el mismo día si se elimina el high-water mark; confusión en cocina. | - |
| **Unicidad de numeroOrden** | Garantizada por la transacción exclusiva de SQLite que serializa las asignaciones: se obtiene el MAX histórico del día (activos + cancelados) y se incrementa en 1. No se usa gap-filling. | `lib/core/database/db_helper.dart:obtenerSiguienteNumeroOrden` | Colisiones bajo concurrencia si se elimina la transacción exclusiva. | ID 67 |
| **Números cancelados no se reutilizan** | Un número de orden asignado a un pedido cancelado nunca se reasigna en el mismo día. La secuencia continúa desde el máximo histórico (incluyendo cancelados). Ejemplo correcto: 23, 24, 25(cancelado), 26, 27. Ejemplo incorrecto: 23, 24, 25(cancelado), 25, 26. | `lib/core/database/db_helper.dart:_calcularSiguienteNumeroOrden` | Confusión en cocina si un número cancelado reaparece; auditabilidad comprometida. | - |
