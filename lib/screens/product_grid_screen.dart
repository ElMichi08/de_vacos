        ...
        if (isLandscape || isTablet) {
          return GridView.builder(
            ...
            gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
              crossAxisCount: isLandscape && constraints.maxWidth > 1000 ? 3 : 2,
        ...
        endActionPane: ActionPane(
          ...
          children: [
            SlidableAction(
              ...
              backgroundColor: const Color(0xFFC62828), // Mismo color que pedidos
          ...
          child: ListTile(
            ...
            onTap: () async {
              final result = await Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => AddProductScreen(producto: producto),
                ),
              );
              if (result == true) {
                _cargarProductos();
  ...
  Future<void> _eliminarProducto(Producto producto) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Producto "${producto.nombre}" eliminado permanentemente'),
            backgroundColor: AppColors.success,
            duration: const Duration(seconds: 2),
          ),
        );
